        ...
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div>
                    ...
                    <p class="text-dark-400 mt-2">مدیریت و پیگیری درخواست‌های تعمیر، سرویس و نصب</p>
                </div>
            ...
            <div class="card-luxury p-6">
                <form method="GET" action="{{ route('workrequests.index') }}" class="flex flex-col lg:flex-row gap-4">
                    <div class="flex-1">
                        <div class="relative">
                            <input type="text"
                                name="search"
                                value="{{ request('search') }}"
                                placeholder="جستجو در شماره درخواست، سریال، مدل، تجهیز یا نام تکنسین..."
                    ...
                    <select name="request_type" class="input-luxury lg:w-48">
                    ...
                    <a href="{{ route('workrequests.index') }}" class="btn-secondary lg:w-auto">
                        حذف فیلتر
                    </a>
                    @endif
                </form>
                ...
                <div class="card-luxury p-6 hover:shadow-2xl hover:shadow-primary-900/20 hover:scale-[1.02] transition-all duration-300 group">
                    ...
                    <div class="flex items-start justify-between mb-4 pb-4 border-b-2 divider">
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-dark-400 mt-1">درخواست: {{ $request->request_number }}</p>
                        </div>
                        ...
                        <div class="flex flex-col gap-2 items-end">
                            ...
                            <span class="px-3 py-1 rounded-full text-xs font-bold border {{ $typeConfig[0] }}">
                                {{ $typeConfig[1] }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="flex items-center gap-2 text-sm">
                           ...
                           <span class="text-cream-200 font-medium">{{ $request->request_date_jalali }}</span>
                        </div>
                    ...
                    <div class="section-inner mb-4">
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <div class="text-xs text-dark-400">مسئول تماس:</div>
                                <div class="text-cream-100 font-medium mt-1">{{ $request->contact_person }}</div>
                            </div>
                            <div>
                                <div class="text-xs text-dark-400">تلفن:</div>
                                <div class="text-cream-100 font-medium mt-1 direction-ltr text-right">{{ $request->contact_phone }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-inner mb-4">
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <div>
                                <span class="text-dark-400">برآورد:</span>
                                <span class="text-yellow-400 font-bold">{{ number_format($request->estimated_cost) }} ریال</span>
                            </div>
                            @endif
                            @if($request->final_cost)
                            <div>
                                <span class="text-dark-400">نهایی:</span>
                                <span class="text-green-400 font-bold">{{ number_format($request->final_cost) }} ریال</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 mb-4 flex-wrap">
                        ...
                        ['label' => 'تامین', 'status' => $request->supply_approval],
                    ...
                    <div class="flex gap-2 pt-4 border-t-2 divider">
                        <a href="{{ route('workrequests.show', $request) }}"
                        ...
                        @if(auth()->id() == $request->user_id && in_array($request->status, ['new', 'pending']))
                        <a href="{{ route('workrequests.edit', $request) }}"
                        ...
                        <form action="{{ route('workrequests.destroy', $request) }}"
                            method="POST"
                            onsubmit="return confirm('آیا از حذف این درخواست اطمینان دارید؟')">
                            @csrf @method('DELETE')
                            <button type="submit"
